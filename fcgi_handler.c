                                 ...
                                 char *val, int val_len, int val_free)
{
    ...
    char buffer2[512] = {0,};
    memcpy(buffer2, val, val_len);
    MK_TRACE("add http header param %s: %s", buffer,buffer2);
...
static inline int fcgi_add_param_net(struct fcgi_handler *handler)
{
    ...
    if (addr.ss_family == AF_INET) {
        ...
        if (IN6_IS_ADDR_V4MAPPED(&s->sin6_addr)) {
            ...
            // For example when we come here it would be like this ::ffff:10.106.146.53
            // This would be converted to 10.106.146.53 
           ...
           inet_ntop(AF_INET, &s4->sin_addr, buffer, sizeof buffer);
        } else {
           inet_ntop(AF_INET6, &s->sin6_addr, buffer, sizeof buffer);
        }
    }
...
static inline int fcgi_stdin_chunk(struct fcgi_handler *handler)
{
    ...
    MK_TRACE("[fastcgi] Bufflen: [%i]",handler->buf_len);
...
static int fcgi_encode_request(struct fcgi_handler *handler)
{
    ...
    /* HTTPS */
    if (MK_SCHED_CONN_PROP(handler->cs->conn) & MK_CAP_SOCK_SSL) {
        fcgi_add_param(handler,
...
int cb_fastcgi_on_read(void *data)
{
    ...
    while (1) {
        ...
        case FCGI_STDOUT:
             ...
             * However, IE works fine as it discards the packets with ZERO size data.
             */
            if (0 != header.content_length) {
                ret = fcgi_response(handler, body, header.content_length);
            }
            else {
                MK_TRACE("[fastcgi=%i] received ZERO byte content length in STDOUT, discard!!",
                         handler->server_fd);
                ret = 0;
            }
            break;
